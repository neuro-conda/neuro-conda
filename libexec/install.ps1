...
If ((-not $CondaIsInstalled) -or ($Env:ncCI)){
    ...
    Start-Process -FilePath "$Env:temp\Miniconda3-latest-Windows-x86_64.exe" -ArgumentList "/S","/InstallationType=JustMe","/D=$CondaInstallationDirectory" -PassThru -Wait
