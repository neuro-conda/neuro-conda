...
If ((-not $CondaIsInstalled) -or ($Env:ncCI)){
    ...
    Invoke-Expression "$Env:temp\Miniconda3-latest-Windows-x86_64.exe /S /InstallationType=JustMe /D=$Env:CondaInstallationDirectory"
    Write-Host "$Env:temp\Miniconda3-latest-Windows-x86_64.exe /S /InstallationType=JustMe /D=$Env:CondaInstallationDirectory"
